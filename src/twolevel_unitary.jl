...
@inline unrows!(state::AbstractMatrix, inds::AbstractVector, U::IMatrix) = state
...
############# uncols #############
Base.@propagate_inbounds function uncols!(
    state::AbstractVector,
    inds::AbstractVector,
    U::AbstractMatrix,
)
...
Base.@propagate_inbounds function uncols!(
    ...
    for k in 1:size(state, 1)
        state[k, inds] = U*state[k, inds]
    end
    return state
end

@inline uncols!(state::AbstractMatrix, inds::AbstractVector, U::IMatrix) = state

Base.@propagate_inbounds function uncols!(state::AbstractMatrix, inds::AbstractVector, U::SDDiagonal)
    for i in 1:length(U.diag)
        for j in 1:size(state, 1)
            state[j, inds[i]] *= U.diag[i]
        end
    end
    state
end
...
Base.@propagate_inbounds function uncols!(
    ...
    for k in 1:size(state, 1)
        state[k,inds] = state[k,inds[U.perm]] .* U.vals
    end
    state
end
